...
- 对话页支持一次上传多张题目图片，公网环境通过扣子 `OCR / Image2text`
  识别题目，Windows 本机 OCR 仅作为开发环境降级方案。
...
公网识图必须在服务器环境变量中配置 `COZE_API_TOKEN`、`COZE_BOT_ID`，
并保持 `WINDOWS_OCR_FALLBACK=false`。令牌仅保存在服务器环境变量中，
不要写入代码、`.env.example` 或仓库。
...
- 本地 58 项单元测试通过，公网极限回归 6/6、多章节回归 9/9 通过。
...
图片识别已改用 `OCR / Image2text` 插件，插件返回题目文字和公式。图片题采用严格两阶段流程：第一轮只转写并等待用户确认，确认后的下一轮才调用计算工作流；普通清晰图片和轻微模糊但主体可辨认的图片应正常识别，个别不确定的符号会标注“疑似”并交由用户核对，只有严重模糊、遮挡或关键内容残缺时才要求重新拍照。语音和参赛材料仍在后续开发中。
