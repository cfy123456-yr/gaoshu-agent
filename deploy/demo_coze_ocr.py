...
4. 必须区分容易混淆的符号：数字 0 与字母 O/o、数字 1 与字母 l/I、字母 x 与乘号 ×、负号与减号、导数撇号与指数、积分上下限、绝对值以及向量箭头。
5. 看不清的符号先结合上下文写最可能的读法，并紧跟括号标注“疑似”，例如 0（疑似 O）；不要擅自改正，也不要因为局部不清就省略整道题。
6. 公式使用纯文本表达，例如 x^2、sqrt(x)、pi、lim x->0 sin(x)/x。
7. 不要输出 Markdown、代码块、LaTeX 定界符或额外说明。
8. 只有题目主体严重模糊、关键区域被遮挡或图片不是题目时，才回复：识别可能不准确，请重新上传清晰、完整的题目图片。
""".strip()
